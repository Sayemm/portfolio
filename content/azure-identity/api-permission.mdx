...
Admin consent is the green "Granted for [tenant]" state in the portal. The tenant name in that button — "Granted for Contoso," "Grant admin consent for Contoso" — is the **organization/tenant**, not a person: it means "approve this app across the whole Contoso directory." Consenting once covers every matching resource in the tenant (the Part 2 "same building, one badge" idea), which is exactly why one grant lets the app reach many mailboxes.

**Who can click that button, and why.** Admin consent can only be given by someone holding an admin role — a Global Administrator, or a delegated Application/Cloud Application Administrator. The tenant's own administrator can grant it *for their whole organization*, because that is what the role authorizes. This is also why, on someone else's tenant, a developer who is *not* an admin cannot self-consent an app-only permission — an admin there has to do it. In your own tenant, you are that admin, so you can grant it yourself.

Its absence is a top cause of "I added the permission but calls still fail": the permission was requested but never granted.

> **A note on the default `User.Read`.** Every new app registration comes with `User.Read` (Delegated) already added. It lets a signed-in user read their own profile, is added automatically, needs no admin consent, and is harmless — an app-only workload never uses it. Leave it; the permission that matters for app-only mail is the `Mail.Read` **Application** permission added alongside it. The quickest way to confirm you added the right one: it shows **Type: Application** and **Admin consent required: Yes**, whereas `User.Read` shows **Delegated** and **No**.
