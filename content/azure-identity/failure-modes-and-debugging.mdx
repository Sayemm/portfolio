...
| app-only mail call blocked *despite* having `Mail.Read` | mailbox is outside the app's access-policy scope | add the mailbox to the policy's group — Exchange admin (Part 5) |
...
There is also a third, sneakier variant on the API side: the permission *is* granted, but a **resource-specific scoping policy** excludes the exact resource being accessed. The clearest example is Exchange mail — an app-only identity with `Mail.Read` that reads one mailbox fine but is blocked on another, with a message about a tenant-configured app-only access policy (an Application Access Policy, or its successor RBAC for Applications). Nothing is wrong with the app, the token, or the permission; the mailbox is simply outside the policy's allowed group. The fix is a tenant-configuration change by an Exchange admin, not anything in the code — the tell-tale sign being that the *same code and token* succeed for one resource and fail for another.
